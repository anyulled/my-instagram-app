...
app.get(OAUTH_CALLBACK_PATH, (req, res) => {
    ...
    if (!req.cookies.state) {
        res.status(400).send('State cookie not set or expired. Maybe you took too long to authorize.' +
            ' Please try again.');
...
/**
 * Passes the auth code to your Mobile application by redirecting to a custom scheme URL.
 * This serves as a fallback in case App Link/Universal Links are not supported on the device.
...
app.get(OAUTH_CODE_EXCHANGE_PATH, (req, res) => {
    ...
    oauth2.authCode
        ...
        .then(results => {
            ...
            // Create a Firebase Account and get the custom Auth Token.
            createFirebaseAccount(results.user.id,
                results.user.full_name,
                results.user.profile_picture, firebaseToken)
                .then(firebaseToken => {
                    //TODO send access_token
...
function createFirebaseAccount(instagramID, displayName, photoURL, accessToken) {
    ...
    // Save the access token tot he Firebase Real time Database.
